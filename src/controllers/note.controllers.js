...
import mongoose from "mongoose";
  ...
  const getNoteById = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, noteId, "Invalid ID format"));
  ...
  const createNote = asyncHandler(async (req, res) => {
    ...
    await newNote.save();
    return res.status(201).json(new ApiResponse(201,newNote,{message:"new note created succesfully"}));
  });
